...
def create_gpu_uuid():
    """创建一个全零的GPU UUID用于测试"""
    return [0] * 16  # 16字节的UUID

class UVMTestParameters:
    """UVM测试参数设置类 - 为每个测试设置正确的参数"""
    ...
    def setup_test_params(cmd_id, test_name):
        ...
        if cmd_id == 200:  # GET_GPU_REF_COUNT
            ...
            # NvProcessorUuid gpu_uuid (16字节) + NvU64 ref_count (8字节) + NV_STATUS rmStatus (4字节) + 对齐
            # 设置一个全零UUID（可能会失败，但这是合理的测试）
            return params, 28  # rmStatus offset
            
        elif cmd_id == 201:  # RNG_SANITY
            params = array.array('B', [0] * 8)
            # 只有 NV_STATUS rmStatus
            return params, 0
            ...
            return params, 500  # rmStatus在结构末尾
            ...
            # 两个GPU UUID (各16字节) + NV_STATUS (4字节) + NvU64 ref_count (8字节) + 对齐
            return params, 32  # rmStatus在两个UUID之后
            
        elif cmd_id == 208:  # VA_RANGE_SPLIT
            params = array.array('B', [0] * 16)
            # NvU64 split_address + NV_STATUS rmStatus
            struct.pack_into('<Q', params, 0, 0x500000)  # 5MB地址
            return params, 8
            
        elif cmd_id == 209:  # VA_RANGE_INJECT_SPLIT_ERROR
            params = array.array('B', [0] * 16)
            # NvU64 lookup_address + NV_STATUS rmStatus
            struct.pack_into('<Q', params, 0, 0x600000)  # 6MB地址
            return params, 8
            
        elif cmd_id == 210:  # PAGE_TREE
            params = array.array('B', [0] * 4096)  # 复杂的页表测试
            return params, 4092  # rmStatus在末尾
            ...
            # 设置GPU UUID和地址
            struct.pack_into('<Q', params, 16, 0x800000)  # virtual_address
            return params, 124
            ...
            params = array.array('B', [0] * 32)
            # 设置查找地址
            struct.pack_into('<Q', params, 0, 0x900000)  # lookup_address
            return params, 28
            ...
            params = array.array('B', [0] * 4096)  # 需要大缓冲区
            # 设置查找地址
            ...
            params = array.array('B', [0] * 2048)  # 复杂的VA_BLOCK测试
            # 设置查找地址
            ...
            params = array.array('B', [0] * 16)
            # 设置时间间隔
            struct.pack_into('<Q', params, 0, 1000)  # lapse_ns
            return params, 12
            ...
            params = array.array('B', [0] * 32)
            # 设置重置模式
            struct.pack_into('<I', params, 16, 0)  # reset_mode
            return params, 28
            ...
            params = array.array('B', [0] * 16)
            # 设置策略
            struct.pack_into('<I', params, 0, 0)  # policy
            return params, 12
            ...
            params = array.array('B', [0] * 64)
            # 设置性能测试参数
            struct.pack_into('<I', params, 0, 1000)  # iterations
            return params, 60
            ...
            params = array.array('B', [0] * 16)
            return params, 8  # rmStatus在alloc_size_mask之后
            ...
            return params, 1020  # 假设rmStatus在末尾
...
def run_uvm_test_with_correct_params(cmd_id, test_name):
    ...
    try:
        ...
        try:
            # 获取正确的参数设置
            params, rmstatus_offset = UVMTestParameters.setup_test_params(cmd_id, test_name)
...
def run_all_tests_correctly():
    """运行所有测试，使用正确的参数"""
    
    print("UVM Complete Test Runner - All 97 Tests with Correct Parameters")
    print("===============================================================")
    ...
    print(f"开始运行所有 {len(ALL_UVM_TESTS)} 个测试...")
    print()
    
    for i, (cmd_id, test_name, description, requires_gpu) in enumerate(ALL_UVM_TESTS, 1):
        print(f"[{i:2}/{len(ALL_UVM_TESTS)}] {test_name:35} ", end="", flush=True)
        ...
        if 'error' in result:
            ...
            print(f"[FAIL] {result['status_name']}")
            stats['fail'] += 1
    ...
    print("分析:")
    if stats['pass'] > 0:
        print("✅ 有测试通过，说明参数设置和rmStatus检查正确")
    if stats['fail'] > 0:
        print("✅ 有测试失败，说明内核验证确实在工作")
    if stats['error'] > 0:
        print("⚠️ 有系统错误，可能是权限或硬件问题")
        ...
        for cmd_id, name, desc, gpu in ALL_UVM_TESTS:
            if name == args.test:
                ...
                if 'error' in result:
                    ...
                    print(f"ioctl结果: {result['ioctl_result']}")
