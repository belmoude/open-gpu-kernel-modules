...
def setup_test_params(cmd_id, test_name):
    ...
    if cmd_id == 200:  # GET_GPU_REF_COUNT
        ...
        struct.pack_into('<Q', params, 56, 10)
...
class UVMTestRunner:
    ...
    def run_all_tests(self, test_filter=None, continue_on_error=True):
        ...
        print(f"UVM测试运行器 - VA空间增强版 (所有97个测试)")
        print(f"============================================")
        print(f"✅ 包含所有97个测试用例")
        print(f"✅ 添加了VA空间创建功能")
...
def main():
    parser = argparse.ArgumentParser(description="Complete UVM Test Runner with VA Space - All 97 Tests")
    ...
    if args.list:
        ...
        print(f"总共 {len(ALL_UVM_TESTS)} 个测试用例:")
    ...
    if args.test:
        # 运行单个测试的对比
        print(f"单个测试对比: {args.test}")
        print("=" * 50)
        
        # 查找测试
        ALL_UVM_TESTS = [(200, "GET_GPU_REF_COUNT"), (201, "RNG_SANITY"), (218, "LOCK_SANITY"), (230, "MEM_SANITY")]  # 简化版本用于演示
        
        for cmd_id, name in ALL_UVM_TESTS:
            if name == args.test:
                ...
                try:
                    ...
                    if not result1['success'] and result2['success']:
                        print("  🎉 VA空间创建解决了问题!")
                    elif result1['rm_status'] != result2['rm_status']:
